...
const BudgetAnalysis = ({ budget, expenses }) => {
  ...
  const analysisData = categories.map((category) => ({
    ...
    Variance:
      (budget[`${category.key}Budget`] || 0) -
      (expenses[`${category.key}Expense`] || 0),
  }));

  const totalBudgeted = categories.reduce(
    (sum, category) => sum + (budget[`${category.key}Budget`] || 0),
    0
  );

  const totalActual = categories.reduce(
    (sum, category) => sum + (expenses[`${category.key}Expense`] || 0),
    0
  );
    ...
    <Paper elevation={3} sx={{ p: 3, mt: 3 }}>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body1" color={varianceColor}>
          Total Variance: ₹{Math.abs(totalVariance).toLocaleString()} (
          {varianceType})
        </Typography>
        <Typography variant="body1" color={varianceColor}>
